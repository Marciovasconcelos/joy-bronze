...
function relogio(){
  ...
  const horaEl=$("#horaAtual"), centroEl=$("#saudacao"), dataCentroEl=$("#dataHoraCard"), dataEl=$("#dataAtual");
  if(horaEl) horaEl.textContent=hora;
  if(centroEl) centroEl.textContent=hora;
  if(dataCentroEl) dataCentroEl.textContent=data.charAt(0).toUpperCase()+data.slice(1);
  if(dataEl) dataEl.textContent=data;
});$("#dataAtual").textContent=agora.toLocaleDateString("pt-BR",{weekday:"long",day:"2-digit",month:"long",year:"numeric"})}
...
$("#filtroData").value=dataHoje();$("#filtroData").onchange=agenda;const logoutBtn=$("#logout");if(logoutBtn) logoutBtn.onclick=()=>signOut(auth);relogio();setInterval(relogio,1000);
