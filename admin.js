...
import {collection,getDocs,doc,getDoc,query,where,updateDoc,arrayUnion} from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
async function agenda(){const data=$("#filtroData").value||dataHoje();const q=query(collection(db,"agendamentos"),where("data","==",data));const s=await getDocs(q);const lista=s.docs.map(x=>({id:x.id,...x.data()})).sort((a,b)=>(a.horario||"").localeCompare(b.horario||""));const ativos=lista.filter(a=>a.status!=="concluido");const realizados=lista.filter(a=>a.status==="concluido");$("#agenda").innerHTML=ativos.length?ativos.map(cardAgendamento).join(""):"Nenhum agendamento pendente ou confirmado para esta data.";$("#realizados").innerHTML=realizados.length?realizados.map(cardRealizado).join(""):"Nenhum atendimento realizado nesta data.";document.querySelectorAll("[data-status]").forEach(el=>el.onchange=async()=>{const id=el.dataset.status;if(el.value==="confirmado"){await updateDoc(doc(db,"agendamentos",id),{status:"confirmado"});agenda()}else if(el.value==="concluido"){alert("O pagamento precisa ser informado antes de concluir. Selecione Confirmado para abrir o balão de pagamento.");agenda()}else{await updateDoc(doc(db,"agendamentos",id),{status:el.value});agenda()}});document.querySelectorAll("[data-save-pagamento]").forEach(b=>b.onclick=()=>salvarPagamento(b.dataset.savePagamento))}

$("#filtroData").value=dataHoje();$("#filtroData").onchange=agenda;$("#logout").onclick=()=>signOut(auth);relogio();setInterval(relogio,1000);
onAuthStateChanged(auth,async u=>{if(!u){location.href="admin-login.html";return}const p=await getDoc(doc(db,"usuarios",u.uid));if(!p.exists()||p.data().tipo!=="admin"){location.href="index.html";return}agenda();ativarNotificacoes(u)});